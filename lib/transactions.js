...
function getOutputIndices(transactions, config, key, callback){
  var vouts = [];
...
var Transactions = function(config){
  ...
  function Get(transactions, callback){
    ...
    utility.getFromURL(url, function (err, resp){
      if (err || resp.error) {
        ...
        getOutputIndices(responseData, config, key, function (err, done){
  ...
  function Latest(callback){
    var response = {};
    var url = utility.getBaseURL(config.network) + "/txs?includeHex=true&" + key;
    utility.getFromURL(url, function (err, resp){
      if (err){
        callback(err, null);
      } else {
        var transaction = [standardizeTransaction(resp[0])];
        getOutputIndices(transaction, config, key, function (err, done) {
  ...
  function Propagate(transactionHex, callback){
    ...
    var url = utility.getBaseURL(config.network) + "/txs/push?" + key;
